...
def InitArray(Nradi):
    phsc_ = np.load("data/data_cache/transit_tmp/phsc.npy")
    rmax_ = np.load("data/data_cache/transit_tmp/rscmax.npy")
    rmin_ = np.load("data/data_cache/transit_tmp/rscmin.npy")
    ...
    np.save("data/data_cache/transit_tmp/Xsc.npy", Xsc)
    np.save("data/data_cache/transit_tmp/Ysc.npy", Ysc)
    np.save("data/data_cache/transit_tmp/Rsc.npy", Rsc)
    np.save("data/data_cache/transit_tmp/Psc.npy", Psc)

    dSsc = dSscInit(Rsc, 2 * np.pi / Nph)
    np.save("data/data_cache/transit_tmp/dSsc.npy", dSsc)
...
def Save_data(spin, inc):
    dSsc = np.load("data/data_cache/transit_tmp/dSsc.npy")
    # RadRed = np.load("data/RadiRedsh2d.npy")
    RadRed = np.load("data/data_cache/transit_tmp/radi_redsh2d.npy")
    ...
    fname = "data/data_cache/transit/transit_data_dic_a%.3f_inc_%d.npz" % (spin, inc)
...
def TransitRay(Npar, spin, inc, cache = 0):
    ...
    if check == False or cache==0:
        ...
        ph0 = -np.pi / 2 + 0.001
        ph1 = 2.0 * np.pi - np.pi / 2 + 0.001
        # phsc = np.linspace(ph0, ph1, Nphsc, endpoint=True)
        phsc = np.linspace(ph0, ph1, Nphsc, endpoint=False)

        # ph0 = -np.pi/2 + 0.01
        # ph1 = 2.*np.pi - np.pi/2 + 0.01 - 0.0001
        # phsc = np.linspace(ph0, ph1, Nphsc)
        np.save("data/data_cache/transit_tmp/phsc.npy", phsc)

        RscGen(0, spin, inc, "data/data_cache/transit_tmp/rscmax.npy")
        RscGen(1, spin, inc, "data/data_cache/transit_tmp/rscmin.npy")

        InitArray(Nradi)

        # RayGen(spin, inc, "data/RadiRedsh2d.npy", Npar)
        RayGen(spin, inc, "data/data_cache/transit_tmp/radi_redsh2d.npy", Npar)
